...
class BookDatasetSeederTest extends TestCase
{
    ...
    public function test_seeder_loads_real_catalog_dataset_with_relationships_and_copies(): void
    {
        ...
        $this->assertSame($summary['books'], Book::query()->count());
        $this->assertGreaterThan(20, Author::query()->count());
        $this->assertGreaterThan(10, Category::query()->count());
        $this->assertGreaterThanOrEqual(1, Publisher::query()->count());
        ...
        if ($book) {
            ...
            $this->assertTrue($book->categories->contains('name', 'Romance') || $book->categories->contains('name', 'Fiction') || $book->categories->isNotEmpty());
            $this->assertGreaterThanOrEqual(1, $book->copies->count());
    ...
    {
        $seeder = new BookDatasetSeeder;
        $firstSummary = $seeder->run();
        $summary = $seeder->run();

        $this->assertSame(0, $summary['books']);
        $this->assertSame(0, $summary['copies']);
        $this->assertSame($firstSummary['books'], Book::query()->count());
